...
@interface LJContactManager () <ABNewPersonViewControllerDelegate, CNContactViewControllerDelegate,UIAlertViewDelegate>

@property (nonatomic, copy) void (^handler) (NSString *name, NSString *phone);
@property(nonatomic,copy) void (^handlerBlock)(LJPerson *people);  // 回调的block
...
- (LJPickerDetailDelegate *)pickerDetailDelegate
{
    if (!_pickerDetailDelegate)
    {
        ...
        _pickerDetailDelegate.handleBlock = ^(LJPerson *person) {
...
// 获取联系人详细的信息
- (void)selectedContactAtController:(UIViewController *)controller complection:(void (^)(LJPerson *person))completcion;
{
    self.isAdd = NO;
    [self _presentFromController:controller];
